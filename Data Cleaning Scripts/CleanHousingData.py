...
cleanedHousingDataFrame = housingDataFrame.dropna(subset=['id', 'url', 'price', 'type', 'sqfeet', 'beds', 'baths', 'lat', 'long']).drop(
    columns=['id', 'url', 'region', 'region_url', 'state'])
...
cleanedHousingDataFrame['type'] = cleanedHousingDataFrame['type'].replace(
    ['apartment', 'condo', 'house', 'duplex', 'townhouse', 'loft', 'manufactured', 'cottage/cabin', 'flat', 'in-law', 'land', 'assisted living'], [i for i in range(len(housingTypes))])

cleanedHousingDataFrame = cleanedHousingDataFrame.drop_duplicates(
    subset=['lat', 'long'], keep='first')
...
for ind in cleanedHousingDataFrame.index:
    address = requests.get('https://maps.googleapis.com/maps/api/geocode/json?latlng='+str(cleanedHousingDataFrame['lat'][ind])+','+str(
        cleanedHousingDataFrame['long'][ind])+'+&key=AIzaSyBrJrhdPowQtFEeLiX_cdXjc8E9C4IVtyw')
    ...
    if len(jsonAddress["results"]) > 0:
        ...
        if foundNeighbourhood and foundCity and foundCounty and foundState:
            neighbourhoods.append(
                addressComponents[foundNeighbourhoodIndex]["long_name"])
...
cleanedHousingDataFrame = cleanedHousingDataFrame.dropna(
    subset=['neighbourhood', 'city', 'county', 'state', 'address'])
