...
cleanedHousingData = cleanedHousingData.drop_duplicates(
    subset=['address'], keep='first')

housingTypes = cleanedHousingData.type.unique()

cleanedHousingData['Type Category'] = cleanedHousingData['type'].replace([i for i in range(len(housingTypes))], [
                                                                         'apartment', 'condo', 'house', 'duplex', 'townhouse', 'loft', 'manufactured', 'cottage/cabin', 'flat', 'in-law', 'land'])
