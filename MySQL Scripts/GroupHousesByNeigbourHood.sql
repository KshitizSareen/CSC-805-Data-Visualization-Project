...
DELIMITER $$
CREATE PROCEDURE `GroupHousesByNeighborhood` (IN minPrice INT, IN maxPrice INT, IN housingTypes VARCHAR(255), IN minSqFeet INT, 
...
IN in_comes_furnished INT, IN city_array VARCHAR(255))

BEGIN

SELECT AVG(price) as 'avgPrice',Min(price) as 'minPrice',Max(price) as 'maxPrice',COUNT(*) as 'Count',neighbourhood FROM Housing_Data
