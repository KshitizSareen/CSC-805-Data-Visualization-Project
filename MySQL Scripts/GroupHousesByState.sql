...
DELIMITER $$
CREATE PROCEDURE `GroupHousesByState` (IN minPrice INT, IN maxPrice INT, IN housingTypes VARCHAR(255), IN minSqFeet INT, 
...
IN in_comes_furnished INT, IN state_array VARCHAR(255))

BEGIN

SELECT AVG(price) as 'avgPrice',Min(price) as 'minPrice',Max(price) as 'maxPrice',COUNT(*) as 'Count',state FROM Housing_Data
