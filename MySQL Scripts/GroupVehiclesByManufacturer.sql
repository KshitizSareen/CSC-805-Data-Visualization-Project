...
DELIMITER $$
CREATE PROCEDURE `GroupVehiclesByManufacturer` (IN minPrice INT, IN maxPrice INT, IN minYear INT, IN maxYear INT, 
...
SELECT AVG(price) as 'avgPrice',Min(price) as 'minPrice',Max(price) as 'maxPrice',COUNT(*) as 'Count',`manufacturer` FROM Vehicle_Data
