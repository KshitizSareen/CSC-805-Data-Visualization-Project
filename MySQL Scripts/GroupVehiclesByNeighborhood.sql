...
DELIMITER $$
CREATE PROCEDURE `GroupVehiclesByNeighborhood` (IN minPrice INT, IN maxPrice INT, IN minYear INT, IN maxYear INT, 
...
SELECT AVG(price) as 'avgPrice',Min(price) as 'minPrice',Max(price) as 'maxPrice',COUNT(*) as 'Count',neighbourhood FROM Vehicle_Data
