...
DELIMITER $$
CREATE PROCEDURE `GroupVehiclesByState` (IN minPrice INT, IN maxPrice INT, IN minYear INT, IN maxYear INT, 
...
IN in_min_odometer INT, IN in_max_odometer INT, IN state_array VARCHAR(255))

BEGIN

SELECT AVG(price) as 'avgPrice',Min(price) as 'minPrice',Max(price) as 'maxPrice',COUNT(*) as 'Count',state FROM Vehicle_Data
...
GROUP BY state ORDER BY `index` ASC;
